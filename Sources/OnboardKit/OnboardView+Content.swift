...
/**
 * Content
 * - Description: This section of the code extends the OnboardView struct, 
 *                adding a body property and several other components.
 *                The body property contains a ZStack that layers the 
 *                `pageContainer`, `navView`, and `controlOverlay` (for macOS only).
 *                The pageContainer manages the display of onboarding pages and the page control indicator. 
 *                The navView manages the navigation controls for the onboarding process. The controlOverlay, which is only used in macOS, provides navigation buttons for users to manually navigate through the onboarding process.
 * - Fixme: ⚠️️ See old legacy text copy, maybe use some of it?
 */
extension OnboardView {
   /**
    * Body
    * - Description: This is the main body of the OnboardView. 
    *                It consists of a ZStack that layers the 
    *                `pageContainer`, `navView`, and `controlOverlay` (for macOS only).
    *                The pageContainer manages the display of onboarding pages and the page control indicator.
    *                The navView manages the navigation controls for the onboarding process.
    *                The controlOverlay, which is only used in macOS,
...
extension OnboardView {
    ...
    * - Description: The `PageContainer` view manages the display of onboarding pages and the page control indicator. 
    *                It binds to the `currentPageIndex` to update the currently visible page and uses `pageModels` to populate the content of each page.
    ...
    * - Description: The `ControlOverlay` is designed specifically for macOS, as it provides navigation buttons for users to manually navigate through the onboarding process. This is particularly useful on macOS, where swipe gestures are not as intuitive as they are on iOS devices. The overlay includes both previous and next buttons, allowing users to move through the onboarding pages at their own pace.
    ...
    *                providing a consistent navigation experience across both iOS and macOS.
    *                However, there are some subtle differences in its behavior and appearance between the two platforms.
    *                On iOS, the `PagerNavView` is optimized for touch-based navigation,
    *                with larger tap targets and a more prominent action button. On macOS,
    *                the `PagerNavView` is tailored for mouse and keyboard navigation,
    *                with a more compact layout and a focus on keyboard accessibility.
    *                Despite these differences, the core functionality of navigating through onboarding pages
    *                and completing the onboarding process remains consistent across both platforms.
    */
