 ...
 * - Description: This method checks the current platform and device type
 *                to apply a closure to the view. It supports conditional
 *                application for iOS devices, including iPads and iPhones,
 *                as well as macOS devices.
 * - Note: This method is particularly useful for making platform-specific
 *         adjustments to your SwiftUI views, especially when you need to
 *         apply different styling or behavior on different devices or
 *         platforms.
 * - Parameter closure: A closure that takes the current view as a parameter
 *                      and returns a view. This closure is applied to the
 *                      view based on the platform and device type.
 * - Returns: The view with the closure applied if the condition is met,
 *            otherwise the original view.
 */
extension View {
   /**
    * Conditionally applies a closure to the view if the device is an iPad or a Mac.
    * - Description: This method checks if the current platform is iOS and if
    *                the device is an iPad. If both conditions are true, it
    *                applies the provided closure to the view. If the platform
    *                is macOS, it also applies the closure.
    * - Note: This method is particularly useful for making platform-specific
    *         adjustments to your SwiftUI views, especially when you need to
    *         apply different styling or behavior on iOS devices.
    * ## Examples:
    * Text("Hello, World!")
    *    .isMacOrIPad {
    *       $0.font(.largeTitle)
    ...
    * - Parameter closure: A closure that takes the current view as a parameter
    *                       and returns a view. This closure is applied to the
    *                       view if the platform is iOS and the device is an iPad,
    *                       or if the platform is macOS.
    * - Returns: The view with the closure applied if the platform is iOS and the
...
extension View {
    ...
    * - Abstract: Presents a sheet that can be either full screen or not,
    *             based on the `isFullScreen` parameter.
    * - Description: This modifier conditionally presents a full-screen cover
    *                or a regular sheet based on the `isFullScreen` parameter.
    *                It allows for a flexible presentation style that can be
    *                adapted for different device types or user interface
    *                requirements.
    * - Note: Useful for cases where on iPhone a fullscreen sheet is better,
    *         but on iPad it looks better with a sheet that is not fullscreen
    * - Note: Use `UIDevice.isiPhone` `UIDevice.isiPad` etc to toggle
    *         fullscreen or not etc
    * - Note: There are examples for this. for instance in HistoryPreview
    * - Note: Given the current state of SwiftUI for macOS, if you need a sheet
    *         that dynamically resizes based on its content, you might consider
    *         implementing a custom solution that programmatically calculates and
    *         sets the sheet's size based on its content. This could involve
    *         measuring the content's size and then adjusting the sheet's size
    *         accordingly, although such an implementation would likely require
    *         significant customization and might not align perfectly with
    *         SwiftUI's declarative nature.
    * - Note: A workaround for managing sheet resizing involves using the
    *         .fixedSize() modifier on a VStack containing the sheet's content,
    *         which helps in adjusting the internal layout to fit the content's
    *         natural size but does not inherently make the sheet resizable based
    *         on its content 1.
